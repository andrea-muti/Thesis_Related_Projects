...
public class ResponseTimeReader {
	...
	public static void main(String[] args){
		...
		try {
			resultFileWriter = new FileWriter(dir_path+"/"+operation+"_response_time_results_instant.csv");
        ...
        for(int i = 0; i<samplesCount; i++){
        	...
        	// CALCULATE LATENCY
			Utils.collectLatencyStatistics(start_op, latencies);
        	...
        	if( System.currentTimeMillis() - now >= update_interval_msec ){
				...
				CurrentAverageLatency cal = Utils.perSecondLatencyAverage(latencies);
				
				try {
					resultBufferedWriter.write(cal.getTime()+DELIMITER+time++ +DELIMITER+cal.getLatency()+DELIMITER+ cal.getOpsNumber()+DELIMITER+ success+DELIMITER+failed+DELIMITER+"\n");
				} catch (IOException e) {
					System.err.println("error with file writer");
					e.printStackTrace();
				}
