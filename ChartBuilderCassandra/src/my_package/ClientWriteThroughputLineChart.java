...
public class ClientWriteThroughputLineChart extends Application {
    ...
    @Override public void start(Stage stage) {
        ...
        stage.setTitle("Client Write Throughputs");

        final CategoryAxis xAxis = new CategoryAxis();
        final NumberAxis yAxis = new NumberAxis();

        xAxis.setLabel("LOAD [GB]");
		yAxis.setLabel("Client Write Throughput [events/sec]");

        final LineChart<String,Number> lineChart = new LineChart<String,Number>(xAxis,yAxis);
       
        lineChart.setTitle("Client Write Throughput vs Cluster Load");
    ...
    private void add_line_to_chart(LineChart<String, Number> lineChart, String file_path) {    	
    	...
    	// si assume che i data file abbiamo name : client_write_throughputs_x_nodes.txt
    	int ind = file_path.lastIndexOf("/")+1;
    	
    	
    	String series_name = file_path.substring(ind).replace("client_write_throughputs_" , "").replace("_nodes", " nodes")
    								 .replace(".txt", "");
		...
		try {
			...
			// line format : 3.0 GB : 3234234.20 events/sec
