...
public class StreamingTimesVsLimitLineChart extends Application {
    ...
    @Override public void start(Stage stage) {
        ...
        stage.setTitle("Streaming Times VS Throughput Limit");

        final CategoryAxis xAxis = new CategoryAxis();
        final NumberAxis yAxis = new NumberAxis();

        xAxis.setLabel("Throughput Limit [Mbps]");
		yAxis.setLabel("Streaming Time [sec]");

        final LineChart<String,Number> lineChart = new LineChart<String,Number>(xAxis,yAxis);
       
        lineChart.setTitle("Streaming Times vs Throughput Limit");
    ...
    private void add_line_to_chart(LineChart<String, Number> lineChart, String file_path) {    	
    	...
    	// si assume che i data file abbiamo name : /home/..../data_to_plot.txt.txt
    	int ind = file_path.lastIndexOf("/")+1;
    	
    	String series_name = file_path.substring(ind).replace(".txt", "");
		...
		try {
			...
			while(line!=null){
				...
				String throughput_label = st.nextToken();
				int value = Integer.parseInt(st.nextToken());
				
			    series.getData().add(new XYChart.Data<String, Number>(throughput_label, value));
