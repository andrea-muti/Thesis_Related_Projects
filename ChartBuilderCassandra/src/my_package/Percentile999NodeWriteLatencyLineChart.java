...
public class Percentile999NodeWriteLatencyLineChart extends Application {
    ...
    @Override public void start(Stage stage) {
        ...
        stage.setTitle("Node Write Latencies at 99.9th percentile");

        final CategoryAxis xAxis = new CategoryAxis();
        final NumberAxis yAxis = new NumberAxis();

        xAxis.setLabel("LOAD [GB]");
		yAxis.setLabel("Node Write Latencies at 99.9th percentile [milliseconds]");

        final LineChart<String,Number> lineChart = new LineChart<String,Number>(xAxis,yAxis);
       
        lineChart.setTitle("Node Write Latency vs Cluster Load");
...
XYChart.Series<String, Number> series = new XYChart.Series<String, Number>();
    	...
    	// si assume che i data file abbiamo name : percentile_999_node_write_latencies_x_nodes.txt
    	int ind = file_path.lastIndexOf("/")+1;
    	
    	
    	String series_name = file_path.substring(ind).replace("percentile_999_node_write_latencies_" , "").replace("_nodes", " nodes")
    								 .replace(".txt", "");
