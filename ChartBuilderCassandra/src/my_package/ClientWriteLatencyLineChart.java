...
public class ClientWriteLatencyLineChart extends Application {
    ...
    @Override public void start(Stage stage) {
        ...
        stage.setTitle("Client Write Latencies");

        final CategoryAxis xAxis = new CategoryAxis();
        final NumberAxis yAxis = new NumberAxis();

        xAxis.setLabel("LOAD [GB]");
		yAxis.setLabel("Client Write Latencies [milliseconds]");
...
XYChart.Series<String, Number> series = new XYChart.Series<String, Number>();
    	...
    	// si assume che i data file abbiamo name : client_write_latencies_x_nodes.txt
    	int ind = file_path.lastIndexOf("/")+1;
    	
    	
    	String series_name = file_path.substring(ind).replace("client_write_latencies_" , "").replace("_nodes", " nodes")
    								 .replace(".txt", "");
