...
public class NodeReadThroughputLineChart extends Application {
    ...
    @Override public void start(Stage stage) {
        ...
        stage.setTitle("Node Read Throughputs");

        final CategoryAxis xAxis = new CategoryAxis();
        final NumberAxis yAxis = new NumberAxis();

        xAxis.setLabel("LOAD [GB]");
		yAxis.setLabel("Node Read Throughput [events/sec]");

        final LineChart<String,Number> lineChart = new LineChart<String,Number>(xAxis,yAxis);
       
        lineChart.setTitle("Node Read Throughput vs Cluster Load");
    ...
    private void add_line_to_chart(LineChart<String, Number> lineChart, String file_path) {    	
    	...
    	// si assume che i data file abbiamo name : node_read_throughputs_x_nodes.txt
    	int ind = file_path.lastIndexOf("/")+1;
    	
    	
    	String series_name = file_path.substring(ind).replace("node_read_throughputs_" , "").replace("_nodes", " nodes")
    								 .replace(".txt", "");
