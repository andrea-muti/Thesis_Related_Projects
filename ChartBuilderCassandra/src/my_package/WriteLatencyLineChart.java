...
public class WriteLatencyLineChart extends Application {
    ...
    @Override public void start(Stage stage) {
        ...
        stage.setTitle("Write Latencies");

        final CategoryAxis xAxis = new CategoryAxis();
        final NumberAxis yAxis = new NumberAxis();

        xAxis.setLabel("LOAD [GB]");
		yAxis.setLabel("Write Latencies [milliseconds]");

        final LineChart<String,Number> lineChart = new LineChart<String,Number>(xAxis,yAxis);
       
        lineChart.setTitle("Write Latency vs Cluster Load");
...
XYChart.Series<String, Number> series = new XYChart.Series<String, Number>();
    	...
    	// si assume che i data file abbiamo name : mean_write_latencies_x_nodes.txt
    	int ind = file_path.lastIndexOf("/")+1;
    	
    	
    	String series_name = file_path.substring(ind).replace("mean_write_latencies_" , "").replace("_nodes", " nodes")
    								 .replace(".txt", "");
