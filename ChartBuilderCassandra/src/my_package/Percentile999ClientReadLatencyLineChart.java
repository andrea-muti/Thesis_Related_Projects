...
public class Percentile999ClientReadLatencyLineChart extends Application {
    ...
    @Override public void start(Stage stage) {
        ...
        stage.setTitle("Client Read Latencies at 99.9th percentile");

        final CategoryAxis xAxis = new CategoryAxis();
        final NumberAxis yAxis = new NumberAxis();

        xAxis.setLabel("LOAD [GB]");
		yAxis.setLabel("Client Read Latency at 99.9th percentile [milliseconds]");
    ...
    private void add_line_to_chart(LineChart<String, Number> lineChart, String file_path) {    	
    	...
    	// si assume che i data file abbiamo name : percentile_999_client_read_latencies_x_nodes.txt
    	int ind = file_path.lastIndexOf("/")+1;
    	
    	String series_name = file_path.substring(ind).replace("percentile_999_client_read_latencies_" , "").replace("_nodes", " nodes")
    								 .replace(".txt", "");
